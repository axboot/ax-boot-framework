    ...
    var axClass = function () {
        ...
        var
                        ...
                        <tr>
                            ...
                            <td class="calendar-col-{{@i}}" style="{{itemStyles}}">
                                <a class="calendar-item-day {{addClass}}" data-calendar-item-date="{{thisDate}}">
                                    <span class="addon addon-header">{{addon-header}}</span>
                                    {{thisDataLabel}}
                                    <span class="addon addon-footer">{{addon-footer}}</span>
                                </a>
                            </td>
                            {{/list}}
                        </tr>
            ...
            applyPeriodMap = function(){
                setTimeout((function () {
                    if (cfg.mode === "day" || cfg.mode === "d") {
                        this.$["body"].find('[data-calendar-item-date]').removeClass(cfg.defaultPeriodTheme);
                        for (var k in this.periodMap) {
                            this.$["body"].find('[data-calendar-item-date="' + k + '"]').addClass(cfg.defaultPeriodTheme);
                        }
                    }
                }).bind(this));
        ...
        this.setPeriod = (function(){
            ...
            var processor = {
                ...
                'range': function (v, map) {
                    ...
                    v.range.forEach(function (n) {
                        if (U.isDateFormat(n.from) && U.isDateFormat(n.to)) {
                            for (var d = U.date(n.from); d <= U.date(n.to); d.setDate(d.getDate() + 1)) {
                                if(d == U.date(n.from) || d == U.date(n.to)){
                                    map[U.date(d, {"return": cfg.dateFormat})] = {theme: n.theme, label: n.label};
                                }
                                else{
            ...
            return function (period, isApply) {
                ...
                if (cfg.period = period) {
                    for (key in processor) {
                        if (period[key]) {
                            result = processor[key](period);
                            break;
                        }
                    }
                    if (Object.keys(result).length === 0) {
                        result = processor.obj(period);
                    }
                }

                this.periodMap = result;
                // 변경내용 적용하여 출력
                if (isApply !== false) applyPeriodMap.call(this);
                return this;
            };
        })();
